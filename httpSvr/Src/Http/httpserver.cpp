...
HttpStream::HttpStream(Socket::IClient *iclient): m_client(iclient)
{
	...
	*/
	...
	tracef("constructor socket[%#X]\n", m_client->fd());
}
HttpStream::~HttpStream()
{
	warnf("distructor socket[%#X]\n", m_client->fd());
...
void HttpStream::handle_in(int fd)
{
	...
	if(httprequest.load_packet(m_readbuff, nread) < 0){
		errorf("parse package error\n");
		return;
	}

	tracef("socket[%d] receive <--- %ld bytes\n", fd, nread);
...
{
	tracef("constructor ip %s, port %d\n", addr.c_str(), port);
	warnf("this is message of warning\n");
	errorf("this is a message of error\n");
}

HttpServer::~HttpServer()
{
	tracef("distructor ip %s, port %d\n", m_addr.c_str(), m_port);
}
